...
3. **Migrate the high-priority hooks one-by-one** in this order (each is independently shippable):
   ...
   - `useThreadMutation` → strip `onError: toast.error()`. Add `threadMutations.delete`, `threadMutations.togglePinned`, `threadMutations.abort` to `thread/lib/mutations.ts` (alongside the existing `threadMutations.rename` from Wave 1, and `threadMutations.create` from Phase 3b). Each consumer adds its own `onError` at the call site.
