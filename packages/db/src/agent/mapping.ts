...
): Promise<{ message: SerializedMessage }> {
...
): Promise<{ content: SerializedContent }> {
  ...
  const serialized = await Promise.all(
    content.map(async (part) => {
      ...
      switch (part.type) {
        ...
        case "image": {
          const image = serializeDataOrUrl(part.image);
          return {
            type: part.type,
        ...
        case "file": {
          const data = serializeDataOrUrl(part.data);
          return {
            type: part.type,
            data,
