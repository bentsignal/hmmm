...
// AgentComponent used to be the auto-generated `ComponentApi` from the
// `@convex-dev/agent` component. Now that the agent code is inlined into
// this app, it's the `internal.agent` namespace from our own _generated/api.
// Typed loosely as a deep object of FunctionReferences so we don't need a
// circular import on _generated.
import type {
  FunctionReference,
  GenericActionCtx,
  GenericDataModel,
...
// namespace. Typing it strictly here means we'd need to enumerate every
// query/mutation the inlined client code touches, which is a maintenance
// burden for inlined library code. Use `any` and rely on the runtime
// component.ts shim being correct — runtime errors will be loud and
// immediate during a smoke test if a key is misspelled.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type AgentComponent = any;
// FunctionReference import retained for use elsewhere in this file.
type _UnusedFunctionRefImport = FunctionReference<"query", "internal">;
